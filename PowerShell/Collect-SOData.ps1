...
<#
.Synopsis
 Collect-SOData runs a collection of investigation and response modules to collect forensic data off a host

 .Description
 Collect-SOData runs a collection of investigation and response modules to collect forensic data off a host
 Modules included in Collect-SOData:
    Get-AccessToken -ReturnHashtables
    Get-ArpCache -ReturnHashtables 
    Get-AtomTable -ReturnHashtables 
    Get-InjectedThread 
    #Get-KerberosTicketCache 
    Get-LogonSession -ReturnHashtables 
    Get-MasterBootRecord -ReturnHashtables 
    Get-NetworkConnection -ReturnHashtables 
    Get-PSIProcess -ReturnHashtables 
    Get-PSIScheduledTask -ReturnHashtables 
    Get-PSIService -ReturnHashtables 
    Get-RegistryAutoRun
    Get-SecurityPackage -ReturnHashtables
    Get-WmiEventSubscription
 
 .Parameter CollectionModule
Choose from the list of modules provided in the validateset. Current Command for each available module name:

"All" -- Runs all modules -- ****Proceed with caution****
"AccessToken" -- Get-AccessToken -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"ArpCache" -- Get-ArpCache -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"AtomTable" -- Get-AtomTable -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"InjectedThread" -- Get-InjectedThread | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"KerberosTicketCache" -- Get-KerberosTicketCache | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"LogonSession" -- Get-LogonSession -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"MasterBootRecord" -- Get-MasterBootRecord -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"NetworkConnection" -- Get-NetworkConnection -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"PSIProcess" -- Get-PSIProcess -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"PSIScheduledTask" -- Get-PSIScheduledTask -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"PSIService" -- Get-PSIService -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"RegistryAutoRun" -- Get-RegistryAutoRun | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"SecurityPackage" -- Get-SecurityPackage -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
"WmiEventSubscription" -- Get-WmiEventSubscription | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append

.Parameter FilePath
FilePath can be user specified for a directory that is available and writable on the target host. FilePath will default to "C:\Windows\FData" if not specified.

.Example
$session = Get-PSSession | where {$_.state -eq "opened"}

This step is for PSRemoting Only and will assign any OPEN session on your local host to the SESSION variable for later commands.

.Example
Invoke-Command -FilePath ".\Collect-SOData.ps1" -Session $Session -ArgumentList All

#PowerShell Remoting
- Requires an established PsRemoting Session with the target host
- Save psremoting session to $Session -- See first example
- Full path may be required for -FilePath
- ArgumentList are the arguments you need to pass similar to DATP Live Response and specify Collecton Module first, then OutFilePath second

.Example
Collect-SOData -CollectionModule AccessToken -FilePath C:\Windows\FData

# PowerShell Local Command Steps
- CollectionModule -- Runs Collect-SOData Module "AccessToken"
- FilePath -- Provides a user specified collection directory of "C:\Windows\FData". This is where the output will be saved
- Requires to comment out the function call at the end of the script and the module loaded into your session

.Example
run Collect-SOData.ps1 -parameters "AccessToken C:\Windows\FData"

# DATP Live Response Command Steps
NOTE: Collect-SOData.ps1 must be uploaded into DATP Live Response Library first.
DATP Live Response requires a different approach. We pass the parameters as arguments within DATP Live Response Shell.
- "run" tells DATP to use a loaded script from the library
- "Collect-SOData.ps1" should reflect the name of the loaded script in the DATP Live Response Library
- "parameters" need to be passed as a string of arguments which will be split within the script -- "AccessToken C:\Windows\FData" will be arg[0] arg[1] respectively

.Example
run Collect-SOData.ps1 -parameters "All"

NOTE: Collect-SOData.ps1 must be uploaded into DATP Live Response Library first.
DATP Live Response requires a different approach. We pass the parameters as arguments within DATP Live Response Shell.
- "run" tells DATP to use a loaded script from the library
- "Collect-SOData.ps1" should reflect the name of the loaded script in the DATP Live Response Library
- "All" runs all available modules within the Collect-SOData script -- See HELP Parameter CollectionModule above
- NOTE: The FilePath was not specified and will default to C:\Windows\FData as build in the script

.Link

.Notes
Author(s): Jared Atkinson
Contributor(s): Brandon Scullion, Josh Prager
Date: 12-11-2020
Last Update: 01-21-2021

TODO
    - Document Code
    - Add links to individual/public Modules
    - Modify this script to allow passing a parameter to specify if its running in DATP Live Response or PS Remoting -- Make it required. Notes at bottom of an idea how to do this.

***Running "ALL" parameters "seems" to lock up the DATP Sensor - Further testing needed.
#>
param
(
    [Parameter(Mandatory = $true)]
    [ValidateSet("All","AccessToken","ArpCache","AtomTable","InjectedThread","KerberosTicketCache","LogonSession","MasterBootRecord","NetworkConnection","PSIProcess","PSIScheduledTask","PSIService","RegistryAutoRun","SecurityPackage","WmiEventSubscription")]           
    [string]
    $CollectionModule,
    ...
    $timestamp = $(Get-Date -f MMddyyyy_hhmmsss)
    $TempOutFile = "$TempFilePath\$($hostname)_$timestamp.json"
    $TempOutFileCompressed = "$TempOutFile-compressed.json"
    $FinalOutFile = "$FilePath\$($hostname)_$timestamp-compressed.json"

    switch ($CollectionModule) {
        "AccessToken" 
        {
            Get-AccessToken -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }

        "ArpCache" 
        {
            Get-ArpCache -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
                
        "AtomTable" 
        {
            Get-AtomTable -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "InjectedThread" 
        {
            Get-InjectedThread | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "KerberosTicketCache" 
        {
            Get-KerberosTicketCache | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append # $FilePath -Append
        }
        
        "LogonSession" 
        {
            Get-LogonSession -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "MasterBootRecord" 
        {
            Get-MasterBootRecord -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "NetworkConnection" 
        {
            Get-NetworkConnection -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "PSIProcess" 
        {
            Get-PSIProcess -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "PSIScheduledTask" 
        {
            Get-PSIScheduledTask -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "PSIService" 
        {
            Get-PSIService -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "RegistryAutoRun" 
        {
            Get-RegistryAutoRun | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "SecurityPackage" 
        {
            Get-SecurityPackage -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        
        "WmiEventSubscription" 
        {
            Get-WmiEventSubscription | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
        }
        ...
        "All" 
        {
            ...
            Get-AtomTable -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
            Get-InjectedThread | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
            #Get-KerberosTicketCache | ConvertTo-JsonV2 | Out-File $TempOutFile -Append
            Get-LogonSession -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
            Get-MasterBootRecord -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
            Get-NetworkConnection -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
            Get-PSIProcess -ReturnHashtables | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFile -Append
    ...
    Compress-Content -Content (Get-Content -Path $TempOutFile | Out-String) | ConvertTo-JsonV2 | Out-File -FilePath $TempOutFileCompressed
    Copy-Item -Path $TempOutFileCompressed -Destination $FinalOutFile
    Remove-Item -Path $TempOutFile
    Remove-Item -Path $TempOutFileCompressed
    Return $FinalOutFile # Return full file path to print to screen for DATP Live Response and PSRemoting
}
...
function Compress-Content
{
    ...
    $compressedcontent = [Convert]::ToBase64String($CompressedFileBytes)

    @{
        OriginalContentLength = $Bytes.Length
...
function Get-ArpCache
{
	...
	$Entry.Add('ComputerName', $ComputerName)
...
function Get-KerberosTicketCache
{
    ...
    foreach($Session in $Sessions)
    {
        try
        {
		    ...
		    $t.Add('ComputerName', $ComputerName)
                }
...
#>
    ...
    process
    {
        foreach($Process in $Processes)
        {
            ...
            $FileHash = $null
            if($Path -ne $null -and (Test-Path $Path -ErrorAction SilentlyContinue)) {
...
function Get-RegistryAutoRun
{
    ...
    if (($PSCmdlet.ParameterSetName -ne 'SpecificCheck') -or $PSBoundParameters['Logon'])
    {
        ...
        $null, 'Wow6432Node\' | ForEach-Object {
            ...
            ForEach-Object {
                ...
                $_ | New-AutoRunsEntry -SubKey $InstalledComponents -Name $AutoRunEntryName -Category $Category
            }
        }
        #>
    }
    ...
    if (($PSCmdlet.ParameterSetName -ne 'SpecificCheck') -or $PSBoundParameters['ImageHijacks']) {
        ...
        foreach ($CommonKey in $CommonKeys) {
            Get-RegistryValue -Key "HKLM:\$($CommonKey)" -Value '' | New-AutoRunEntry -Name $CommonKey.Split('\')[2] -Category $Category
            ...
            foreach ($SID in $HKUSIDs) {
                Get-CSRegistryValue -Hive HKU -SubKey "$SID\$CommonKey" -ValueName '' @CommonArgs @Timeout |
                    New-AutoRunEntry -Name $CommonKey.Split('\')[2] -Category $Category
            }
        }
        ...
        $null, 'Wow6432Node\' | ForEach-Object {
            Get-RegistryValue -Key "HKLM:\SOFTWARE\$($_)Microsoft\Windows NT\CurrentVersion\Image File Execution Options" -Value Debugger | 
                ForEach-Object {
                    $_ | New-AutoRunEntry -Name $_.SubKey.Substring($_.SubKey.LastIndexOf('\') + 1) -Category $Category
                }

            Get-RegistryValue -Key "HKLM:\SOFTWARE\$($_)Microsoft\Command Processor" -ValueName Autorun | New-AutoRunEntry -Category $Category
        }
        ...
        if ($Class_exe.ValueContent) {
            ...
            if ($OpenCommand.ValueContent) {
                $OpenCommand | New-AutoRunsEntry -Hive $Class_exe.Hive -SubKey $Class_exe.SubKey -Name $Class_exe.ValueContent -Category $Category
            }
        }
        ...
        if ($Class_cmd.ValueContent) {
            ...
            if ($OpenCommand.ValueContent) {
                $OpenCommand | New-AutoRunsEntry -Hive $Class_cmd.Hive -SubKey $Class_cmd.SubKey -Name $Class_cmd.ValueContent -Category $Category
            }
        }
...
### ADD HERE: Switch for PSRemoting vs DATP Live Response. Something Like
<#
Param (
    [Parameter(Mandatory = $true)]
    [ValidateSet("DATP","PSRemoting")]
    [string]
    $System, #OR $IRSystem OR $IRTool
...
switch {
    ...
    "DATP" {
        $FinalOutFile = Collect-SOData -CollectionModule $args[0] -FilePath $args[1]
        Write-Host "*****TEST OUTPUT: $FinalOutFile"
        Write-Host "[i] Collection File for Module [$($args[0])] should be located on the host at [$($FinalOutFile)]"
        Write-Host "[i] Command for DATP Live Response: fileinfo $($FinalOutFile)"
        Write-Host "[i] Command for DATP Live Response: getfile $($FinalOutFile)"
    }
}

#>

### ONLY TESTED WITH DATP LIVE RESPONSE. May work with PSRemoting if you upload the script with args passed.
$FinalOutFile = Collect-SOData -CollectionModule $args[0] -FilePath $args[1]
Write-Host "*******************************"
Write-Host "[i] Collection File for Module [$($args[0])] should be located on the host at [$($FinalOutFile)]"
Write-host "*******DATP LIVE RESPONSE NEXT COMMANDS*******"
Write-Host "[Command]: fileinfo $($FinalOutFile)"
Write-Host "[Command]: getfile $($FinalOutFile)"
Write-host "*******POWERSHELL REMOTING NEXT COMMANDS*******"
Write-Host "[Command]: copy-item $($FinalOutFile) -Destination "~\Downloads" -FromSession `$Session"
Write-Host "[Command]: Decompress-Content -FileName "~\Downloads\$(($FinalOutFile).split("\")[-1])" -OutFilePath "~/Downloads""
Write-Host "*******************************"
#>